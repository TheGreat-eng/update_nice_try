...
public class AdminController {
    ...
    public ResponseEntity<ApiResponse<Page<AdminUserDTO>>> getAllUsers(
            ...
            @Parameter(description = "Từ khóa tìm kiếm (tên hoặc email)") @RequestParam(required = false, defaultValue = "") String search) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("id").descending());
        Page<User> userPage = userService.findAllUsers(search, pageable);
    ...
    private AdminUserDTO convertToAdminUserDTO(User user) {
        return AdminUserDTO.builder()
                ...
                .deleted(user.getDeleted()) // <<<< THÊM TRƯỜNG NÀY
